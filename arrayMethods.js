...
const totalPopulation = populations.reduce((acc, element) => {a + e})
...
const myStrongest = monstersInYourPocket.filter(element.cp => element.cp > 200)
...
/*
  Use a higher order method to get all the order totals after adding in the sales tax (given to you as a tax rate, hint: you'll 
  need to do some multiplication). Your answer should be an array of numbers, one total for each order.
*/

const ordersTotal = orders.map(function(total){
  return total['price'] + total['price'] * total['tax']
})

const ordersTotal = orders.map(total => total.price + (total.price * total.tax))
